...
- The complement of an event is the collections of all outcomes that **do not** belong the that event. If $A$ is an event, we denote its complement by $A^{C}$.

--

- Suppose that $A$ is the event of rolling a value less than or equal to 2. The $A^{C}$ is the event of rolling a value greater or equal to 3. 
...
- If $P(A)$ is the probability of an event, then $P(A^{C}) = 1 - P(A)$. 
...
- It should see plausible that these probability values are not going to be the same. In fact, the answer to the first question is $\frac{3}{30}\approx 0.43$ (we will see how to compute this soon) and it is $\frac{3}{5}=0.6$ for the second (this should be pretty obvious). 
...
- We can use the law of total probability to compute the probability that we draw a red marble in out die toss marble draw game. Let $A$ be the event that we draw a red marblem, let $B_{1}$ be the event that we roll a 2, and let $B_{2}$ be the event that we do not roll a two, that is, that we roll 1, 3, 4, 5, or 6. Notice that $B_{1}$ and $B_{2}$ are exclusive. Therefore, by the law of total probability

$$
\begin{align*}
P(A) &= P(A|B_{1})P(B_{1}) + P(A|B_{2})P(B_{2}) \\
     &= \frac{3}{5}\frac{1}{6} + \frac{2}{5}\frac{5}{6} \\
...
- With whatever time is left, we will begin to introduce random variables. 
