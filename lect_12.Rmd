...
- Model selection seeks to indentify variables in the model that may not be helpful. 
...
- There are a variety of model selection strategies that are used in practice. We will discuss to of the most common approaches. 
...
- Backward elimination and forward selection use $p$-values in deciding which variables will make up the final model. However, there are other measures that are used in other approaches to model selection. For example, on could seek a model that has the largest adjusted $R^2$ value. Information theoretic measures such as [AIC](https://en.wikipedia.org/wiki/Akaike_information_criterion) and [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion) are also often used. A discussion on these matters falls outside the scope of this course.  
