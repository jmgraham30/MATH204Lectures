...
- This is all a little abstract, so let't think about standarding from a different perspective. 
...
p_1 <- ggplot(data.frame(x = c(-3, 3)), aes(x = x)) +
  ...
  annotate(geom="text",x=-2.1,y=0.3,label="x=-1.5",size=6,color="red")
...
p_2 <- ggplot(data.frame(x = c(-3, 3)), aes(x = x)) +
  ...
  annotate(geom="text",x=2.3,y=0.3,label="x=1.8",size=6,color="red")
