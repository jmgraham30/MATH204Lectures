...
- If $X\sim N(\mu,\sigma)$ is a normal random variable, and if $z^{\ast}$ is the $Z$-score value for $X$ such that $\alpha$ % of the area under the standard normal density curve falls between $-z^{\ast}$ and $z^{\ast}$, then an ** $\alpha$ % confidence interval** for $X$ is 
